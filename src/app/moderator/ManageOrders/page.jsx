...
export default function ModeratorManageOrdersPage() {
  ...
  // Enhanced Auth helpers for moderator
  const getAuthHeaders = (bustCache = false) => {
    let token = localStorage.getItem('auth-token')
    
    if (!token && process.env.NODE_ENV === 'development') {
      // For development, create a temp moderator token
      token = 'temp-moderator-token-for-development'
      console.warn('🔧 Using development temp moderator token')
    }
  ...
  const checkAuth = () => {
    ...
    if (process.env.NODE_ENV === 'development' && !token) {
      console.warn('🔧 Development mode: using temp moderator token')
      // Set default moderator info for development
      const devUser = {
        role: 'moderator',
        branch: 'mirpur', // Default branch for development
        email: 'moderator@dev.com',
        name: 'Dev Moderator'
      }
      setUserInfo(devUser)
      setUserBranch(devUser.branch)
      return true
    }
    ...
    if (storedUserInfo) {
      try {
        ...
        if (user.role !== 'moderator') {
          console.error('❌ Access denied: Not a moderator')
          window.location.href = '/moderator/login'
          return false
        }
        
        if (!user.branch) {
          console.error('❌ Moderator branch not assigned')
          window.location.href = '/moderator/login'
          return false
        }
  ...
  // Enhanced Fetch orders with branch restriction for moderator
  const fetchOrders = useCallback(async (bustCache = false) => {
    ...
    try {
      ...
      // Moderators can only see orders from their branch
      if (userBranch) {
        params.append('branch', userBranch.toLowerCase())
      }
      ...
      console.log('🔍 Moderator fetching orders with params:', Object.fromEntries(params))
      ...
      if (response.status === 403) {
        ...
        setError('Access denied. You do not have permission to view these orders.')
        return
      }
      ...
      console.log('✅ Orders received for moderator:', {
  ...
  }, [currentPage, filters.status, debouncedSearch, debouncedEmail, userBranch])
  ...
  }, [filters.status, debouncedSearch, debouncedEmail])
  ...
  const handleManualRefresh = async () => {
    ...
    Swal.fire({
      icon: 'success',
      title: 'Refreshed!',
      text: `${userBranch} branch orders refreshed`,
      timer: 1500,
  ...
  // Enhanced Update order status for moderator (same permissions as admin for status changes)
    ...
    try {
      ...
      const response = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          orderId,
          status: newStatus,
          notes: `Status changed from ${currentStatus} to ${newStatus} by moderator (${userBranch} branch)`
        })
      })
  ...
  // Enhanced Delete/Cancel order for moderator
  ...
  // Enhanced view order details with product images (same as admin but with moderator context)
          ...
          ${item.availableBranches?.length ? `
            <div style="margin-top: 8px;">
              ${item.availableBranches.map(branch => `
                <span style="background: ${branch === userBranch ? '#dcfce7; color: #16a34a' : '#ddd6fe; color: #7c3aed'}; padding: 2px 8px; border-radius: 12px; font-size: 12px; margin-right: 4px;">
                  ${branch === userBranch ? `📍 ${branch}` : branch}
                </span>
              `).join('')}
            </div>
          ` : ''}
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 25px; margin-bottom: 25px;">
            ...
            <div style="background: #f0f9ff; padding: 20px; border-radius: 12px;">
              ...
              <div style="space-y: 8px;">
                ...
                <p style="margin: 8px 0;"><strong>Branches:</strong> ${order.availableBranches?.map(branch => `<span style="background: ${branch === userBranch ? '#dcfce7; color: #16a34a' : '#ddd6fe; color: #7c3aed'}; padding: 2px 6px; border-radius: 8px; font-size: 11px; margin-right: 4px;">${branch === userBranch ? `📍 ${branch}` : branch}</span>`).join('') || 'None'}</p>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-blue-50 to-teal-100 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="flex justify-between items-start">
              ...
              <div className="flex items-center gap-4">
                <p className="text-gray-600 flex items-center gap-2">
                  ...
                  <span className="font-semibold text-green-700 capitalize">
                    {userBranch || 'Loading...'} Branch
                  </span>
                  <span className="w-1 h-1 bg-gray-400 rounded-full"></span>
                  <span className="font-semibold text-purple-600">{totalOrders} orders</span>
                </p>
              </div>
            </div>
            ...
            <motion.button
              ...
              className="flex items-center gap-2 px-6 py-3 bg-white text-green-600 rounded-xl shadow-lg hover:bg-green-50 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed border border-green-100"
            >
        ...
        {/* Enhanced Filters (No branch filter for moderator - they see only their branch) */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div className="relative group">
              <Search size={20} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-hover:text-green-500 transition-colors" />
              <input
                type="text"
                name="search"
                ...
                className="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-gray-200 focus:border-green-500 focus:ring-4 focus:ring-green-500/10 outline-none transition-all duration-200 bg-white/70"
              />
            </div>

            {/* Search by Customer Email */}
            <div className="relative group">
              <Mail size={20} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-hover:text-green-500 transition-colors" />
              <input
                type="text"
                name="customerEmail"
            ...
            <div className="relative">
              <select
                ...
                onChange={handleFilterChange}
                className="w-full p-3 rounded-xl border-2 border-gray-200 focus:border-green-500 focus:ring-4 focus:ring-green-500/10 outline-none transition-all duration-200 bg-white/70 appearance-none cursor-pointer"
              >
          ...
          <div className="flex justify-between items-center pt-4 border-t border-gray-100">
            <div className="text-sm text-gray-600 flex items-center gap-2">
              ...
              <span>Showing {orders.length} of {totalOrders} orders from</span>
              <span className="px-2 py-1 bg-green-100 text-green-800 rounded-md font-semibold text-xs capitalize">
                {userBranch} Branch
              </span>
            </div>
            ...
            <motion.button
              ...
              className="flex items-center gap-2 px-4 py-2 text-green-600 hover:text-green-700 hover:bg-green-50 rounded-lg font-medium transition-all duration-200"
            >
              <X size={16} />
              Clear Filters
            </motion.button>
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            >
              ...
              <p className="mt-4 text-gray-600 font-medium">Loading {userBranch} branch orders...</p>
            </motion.div>
          ) : error ? (
            <motion.div
              key="error"
            ...
            >
              ...
              <p className="text-gray-600 max-w-md mx-auto">
                {Object.values(filters).some(f => f) 
                  ? `No orders found in ${userBranch} branch matching your filters.`
                  : `No orders have been placed for ${userBranch} branch yet.`}
              </p>
            </motion.div>
          ) : (
            <motion.div
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gradient-to-r from-green-50 to-teal-50 border-b border-gray-200">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left font-bold text-gray-800">Branch Status</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {orders.map((order, index) => {
                      ...
                      const isFromUserBranch = order.availableBranches?.includes(userBranch)
                      ...
                      return (
                        <motion.tr
                          ...
                          className="hover:bg-gradient-to-r hover:from-green-50/50 hover:to-teal-50/50 transition-all duration-200 group"
                        >
                          ...
                          <td className="px-6 py-5">
                            <div className="space-y-2">
                              <div className={`inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-semibold ${
                                isFromUserBranch ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                              }`}>
                                <Store size={10} />
                                {isFromUserBranch ? `📍 ${userBranch} Branch` : 'Multi-Branch Order'}
                              </div>
                              {order.availableBranches?.length > 1 && (
                                <div className="text-xs text-gray-500">
                                  Also in: {order.availableBranches.filter(b => b !== userBranch).join(', ')}
                                </div>
                              )}
                            </div>
                          </td>
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="relative">
                                <select
                                  ...
                                  className="w-full text-xs border-2 border-gray-200 rounded-lg shadow-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 px-3 py-2 font-medium bg-white/90 hover:bg-white transition-all duration-200 appearance-none cursor-pointer"
                                >
                                  {ORDER_STATUSES.map((status) => (
                                    <option key={status.value} value={status.value}>
                                      {status.label}
                          ...
                          <td className="px-6 py-5">
                            <div className="flex gap-2">
                              <motion.button
                                ...
                                className="px-4 py-2 bg-gradient-to-r from-green-500 to-teal-600 text-white rounded-lg hover:shadow-lg flex items-center gap-2 text-sm font-medium transition-all duration-200"
          ...
          >
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
              <div className="text-sm text-gray-600 flex items-center gap-2">
                ...
                <span className="font-bold text-green-600">{totalOrders}</span> orders
                <span className="text-xs text-green-600 font-medium">({userBranch} branch)</span>
              </div>
                ...
                {generatePageNumbers().map((pageNum, index) => (
                  <motion.button
                    ...
                    className={`px-4 py-2 text-sm font-medium border-t-2 border-b-2 border-gray-200 transition-all duration-200 ${
                      pageNum === currentPage
                        ? 'bg-gradient-to-r from-green-500 to-teal-600 text-white border-green-500 shadow-lg'
                        : pageNum === 'ellipsis'
