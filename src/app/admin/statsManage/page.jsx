...
export default function AdminSalesManagement() {
  ...
  const [topSellingBranch, setTopSellingBranch] = useState('')
  ...
  const fetchSalesData = async (showRefresh = false) => {
    ...
    try {
      ...
      if (data.success && data.sales) {
        console.log('✅ Sales data fetched:', data.sales.length, 'sales')
  ...
  const processSalesData = (sales) => {
    ...
    sales.forEach(sale => {
      ...
      // Extract branches from items array
      if (sale.items && Array.isArray(sale.items)) {
        const branchesInSale = new Set()
        
        sale.items.forEach(item => {
          if (item.branch) {
            const branchName = item.branch.toLowerCase()
            branchesInSale.add(branchName)
            branchesSet.add(branchName)
          }
        })

        branchesInSale.forEach(branch => {
          stats[branch] = (stats[branch] || 0) + saleAmount
        })
      }
    ...
    // Find top selling branch
    let topBranch = ''
    let maxSales = 0
    Object.entries(stats).forEach(([branch, amount]) => {
      if (amount > maxSales) {
        maxSales = amount
        topBranch = branch
      }
  ...
  const deleteSale = async (saleId) => {
    ...
    try {
      ...
      const response = await fetch(`/api/sales?saleId=${encodeURIComponent(saleId)}`, {
        method: 'DELETE',
        headers: getAuthHeaders()
      })
      ...
      if (data.success) {
        Swal.fire({
          icon: 'success',
          title: 'Sale Deleted!',
          text: 'Sale record has been permanently deleted.',
  ...
  const exportSalesData = () => {
    const csvContent = [
      ['Sale ID', 'Date', 'Customer', 'Branch', 'Amount', 'Payment Type', 'Status'],
      ...salesData.map(sale => [
        sale.saleId,
        new Date(sale.createdAt).toLocaleDateString(),
        sale.customer?.name || 'N/A',
        sale.items?.map(item => item.branch).join(', ') || 'N/A',
        sale.totalAmount,
        sale.paymentType,
        sale.status
      ])
  ...
  const branchPieData = Object.entries(branchStats).map(([branch, amount], index) => ({
        ...
        >
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <motion.div variants={cardVariants} className="flex-1">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <motion.div
                      ...
                      transition={{ duration: 2, repeat: Infinity }}
                      className="px-2 py-1 bg-gradient-to-r from-purple-100 to-indigo-100 rounded-full"
                    >
              ...
              >
                Comprehensive sales analytics with full CRUD operations. Monitor performance, 
                manage transactions, and gain insights across all branches.
              </motion.p>
            </motion.div>
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-gray-600">Filter and search through sales data with precision</p>
              </div>
            </div>
                  ...
                  >
                    ...
                    >
                      <div className="relative z-10">
                        ...
                        <p className="text-orange-100 font-medium">Top Branch</p>
                      </div>
                    </motion.div>
                  </motion.div>
                      ...
                      <div className="flex items-center justify-between mb-8">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600">Revenue by location</p>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="p-8 border-b border-gray-100">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h3 className="text-xl font-bold text-gray-900">Sales Transactions</h3>
                            <p className="text-gray-600">Complete transaction history with actions</p>
                          </div>
                        </div>
                      ...
                      <div className="text-center py-20">
                        ...
                        >
                          ...
                          <p className="text-gray-600 max-w-md mx-auto">
                            No sales transactions found matching your filters. Try adjusting your search criteria.
                          </p>
                        </motion.div>
                      </div>
                    )}
                    ...
                    >
                      <div className="flex items-center justify-between mb-8">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h3 className="text-xl font-bold text-gray-900">Market Share</h3>
            ...
            >
              <div className="p-6">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-gray-900">Delete Sale</h3>
                ...
                <div className="bg-gray-50 rounded-lg p-4 mb-6">
                  ...
                  <p className="font-semibold text-gray-900">{saleToDelete.saleId}</p>
                  <p className="text-sm text-gray-600">
                    Amount: ${saleToDelete.totalAmount} • Date: {new Date(saleToDelete.createdAt).toLocaleDateString()}
                  </p>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <motion.button
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    onClick={() => deleteSale(saleToDelete.saleId)}
                    disabled={deleting}
